% trial9winter.m --> Based on trial9.m
% The file contain computation do predict PM2.5 and PM10 in the winter
...
if isempty(filtered_winter)
    ...
    % IQR method on the winter period
    C_IQR_winter = C_winter;
    num_vars_winter = width(C_IQR_winter);
    % IQR on summer period
    for i = 1:num_vars_winter
        column_winter = C_IQR_winter(:, i);
        [lower_bound_winter, upper_bound_winter] = iqr_bounds(column_winter);
        C_IQR_winter = C_IQR_winter(column_winter >= lower_bound_winter & column_winter <= upper_bound_winter, :);
    end
...
% C_winter -- C containing only summer peroid
...
m_winter = size(C_IQR_winter);
beg1 = m_winter(1) - kkk - window - hp;
...
for i = 1:kkk-1
    ...
    % Winter
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Cn(i, :) = C_IQR_winter(fin + hp + 1, 1:2);    
    C1 = C_IQR_winter(beg:fin-1, selected_features); 
    y = C_IQR_winter(beg + hp:fin + hp - 1, 1:2)';
    X = [ones(size(y, 2), 1), C1];
    A = X \ y'; % the Moore-Penrose pseudoinverse 
    Ym = X * A;
    dely = y - Ym';
    Xlast = [1, C_IQR_winter(fin + hp + 1, selected_features)];
    ynext(i, :) = Xlast * A;
    error(i, :) = abs(ynext(i, :) - C_IQR_winter(fin + hp + 1, 1:2));
    Ai(i, :, :) = A;
    % ### Normalization:
    Cn_transformed(i, :) = C_winter_normalized(fin + hp + 1, 1:2);
    C1_transformed = C_winter_normalized(beg:fin-1, selected_features); 
    y_transformed = C_winter_normalized(beg + hp:fin + hp - 1, 1:2)';
    ...
    Xlast_transformed = [1, C_winter_normalized(fin + hp + 1, selected_features)];
    ynext_transformed(i, :) = Xlast_transformed * A_transformed;
    error_transformed(i, :) = abs(ynext_transformed(i, :) - C_winter_normalized(fin + hp + 1, 1:2));
...
writetable(error_metrics_table_transposed, 'testwin2/win_error_comparison.csv', 'WriteVariableNames', true, 'WriteRowNames', true);
disp('Transposed Comparison Table of Error Metrics has been saved to testwin2/win_error_comparison.csv');
